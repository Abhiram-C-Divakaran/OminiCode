...
console.log('=== RUNNING WORKSPACE PATH REGRESSION CHECKS ===\n');

try {
  // Existing scope: lexical workspace boundaries only; no authentication assurance.
  ...
  try {
    ...
    escaped1 = true;
    assert(err instanceof Error && err.message.includes('boundary'), 'Traversing upwards with relative paths throws boundary validation error');
  }
  if (!escaped1) {
  ...
  try {
    ...
    escaped2 = true;
    assert(err instanceof Error && err.message.includes('boundary'), 'Inputting absolute system paths throws boundary validation error');
  }
  if (!escaped2) {
